...
fn codegen_msvc_try(
    ...
    let llfn = get_rust_try_fn(bx, &mut |mut bx| {
        ...
        let type_info_vtable = bx.declare_global("??_7type_info@@6B@", bx.type_i8p(AddressSpace::default()));
        let type_name = bx.const_bytes(b"rust_panic\0");
        let type_info =
            bx.const_struct(&[type_info_vtable, bx.const_null(bx.type_i8p(AddressSpace::default())), type_name], false);
