            ...
            Scalar::Ptr(ptr) => {
                let (base_addr, base_addr_space) = match self.tcx.global_alloc(ptr.alloc_id) {
                    GlobalAlloc::Memory(alloc) => {
                        ...
                        (value, AddressSpace::default())
                    }
                    GlobalAlloc::Function(fn_instance) => (self.get_fn_addr(fn_instance), self.data_layout().instruction_address_space),
                    GlobalAlloc::Static(def_id) => {
                        assert!(self.tcx.is_static(def_id));
                        assert!(!self.tcx.is_thread_local_static(def_id));
                        (self.get_static(def_id), AddressSpace::default())
                    }
                };
                let llval = unsafe {
